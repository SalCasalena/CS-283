...
int exec_cmd(cmd_buff_t *cmd) {
    ...
    if (pid == 0) { // Child process
        ...
        if (cmd->input_file) {
            ...
            if (in_fd == -1) {
                fprintf(stderr, "dsh: cannot open input file: %s\n", cmd->input_file);
                exit(EXIT_FAILURE);
            }
        ...
        // Handle output redirection (append mode support)
        if (cmd->output_file) {
            ...
            if (out_fd == -1) {
                fprintf(stderr, "dsh: cannot open output file: %s\n", cmd->output_file);
                exit(EXIT_FAILURE);
            }
...
int execute_pipeline(command_list_t *clist) {
    ...
    for (int i = 0; i < clist->num; i++) {
        ...
        if (pids[i] == 0) {  // Child process
            ...
            // Input redirection (Only for the first command)
            if (i > 0) {
            ...
            // Output redirection (Only for the last command)
            if (i < clist->num - 1) {
