...
int start_server(char *ifaces, int port, int is_threaded) {
    ...
    if (svr_socket < 0) {
        fprintf(stderr, "Error: Failed to boot server\n");
        return svr_socket;
    }
...
int stop_server(int svr_socket){
    if (svr_socket > 0) {
        printf("DEBUG: Closing server socket...\n");
        close(svr_socket);
        svr_socket = -1; // Mark as closed
    }
    return OK;
}
...
int process_cli_requests(int svr_socket){
    ...
    while(1){
        ...
        if (rc == OK_EXIT) {
            printf("DEBUG: Server shutting down gracefully.\n");
 ...
 */
 ...
 int exec_client_requests(int cli_socket) {
    ...
    while (1) {
        ...
        // Read full command (loop until we get a null byte `\0`)
        while (1) {
            recv_size = recv(cli_socket, temp_buff, RDSH_COMM_BUFF_SZ - 1, 0);
            if (recv_size <= 0) {
            ...
            memcpy(io_buff + total_received, temp_buff, recv_size);
        ...
        // **Handle built-in 'exit' and 'stop-server' commands**
        if (strcmp(io_buff, "exit") == 0) {
            printf("DEBUG: Client requested exit\n");
            free(io_buff);
            return OK;
        }
        if (strcmp(io_buff, "stop-server") == 0) {
            printf("DEBUG: Client requested server shutdown\n");
            free(io_buff);
            return OK_EXIT;
        }
        ...
        if (strncmp(io_buff, "cd ", 3) == 0) {
            char *dir = io_buff + 3; // Extract directory path
            char cwd[1024];

            // Trim whitespace
            while (*dir == ' ') dir++;
            if (strlen(dir) == 0) {
                send_message_string(cli_socket, "Error: Missing directory\n");
            ...
            if (chdir(dir) == 0) {
                if (getcwd(cwd, sizeof(cwd)) != NULL) {
                    snprintf(io_buff, RDSH_COMM_BUFF_SZ, "%s\n", cwd);
                } else {
                    snprintf(io_buff, RDSH_COMM_BUFF_SZ, "Directory changed, but unable to retrieve path\n");
                }
            } else {
                snprintf(io_buff, RDSH_COMM_BUFF_SZ, "Error: Failed to change directory (%s)\n", strerror(errno));
            }
        ...
        // **Handle 'echo $?' for last command status**
        if (strncmp(io_buff, "echo $?", 7) == 0) {
            printf("DEBUG: Handling 'echo $?', last_status = %d\n", last_status);
            char status_msg[32];
            snprintf(status_msg, sizeof(status_msg), "%d\n", last_status);
            
            send(cli_socket, status_msg, strlen(status_msg), 0);
            send_message_eof(cli_socket);
            continue;
        }

        // **Execute non-built-in commands**
        FILE *cmd_output = popen(io_buff, "r");
        if (!cmd_output) {
        ...
        while (fgets(output_line, sizeof(output_line), cmd_output)) {
            send(cli_socket, output_line, strlen(output_line), 0);
        }

        last_status = pclose(cmd_output);
        send_message_eof(cli_socket);
    }
...
int send_message_eof(int cli_socket) {
    char eof_marker = RDSH_EOF_CHAR;
    int sent_len = send(cli_socket, &eof_marker, 1, 0);
    ...
    for (int i = 0; i < clist->num; i++) {
        ...
        if (pids[i] == 0) {  // Child process
            ...
            if (i == 0 && clist->commands[i].input_file) {
                ...
                if (in_fd == -1) {
                    perror("rsh: cannot open input file");
                    exit(EXIT_FAILURE);
                }
            ...
            // **Output redirection (`>` or `>>`) for last command**
            if (i == clist->num - 1 && clist->commands[i].output_file) {
                ...
                if (out_fd == -1) {
                    perror("rsh: cannot open output file");
                    exit(EXIT_FAILURE);
                }
